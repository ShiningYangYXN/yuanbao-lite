...
class NodeFsAdapter implements PersistenceAdapter {
  ...
  read(path) { return this.fs.readFileSync(path, "utf-8"); }
  // ...
}

// 浏览器实现（用户自定义）
class BrowserLocalStorageAdapter implements PersistenceAdapter {
  exists(path) { return localStorage.getItem(path) !== null; }
  read(path) { return localStorage.getItem(path) ?? ""; }
  // ...
}
```
