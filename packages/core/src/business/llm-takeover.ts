...
### 单命令授权
- /unsafe allow <命令名> [分钟|forever] — 全局授权单个命令在群聊使用
...
- 受信用户才能开启危险模式或管理信任列表
- /block add <用户ID|*> <范围> — 封禁用户（范围: [all] [llm] [command] 或命令名）
...
可用命令列表会在每次对话时动态注入（见下方"=== 可用命令 ==="），包含每个命令的名称、别名、参数、权限状态。请不要猜测命令参数，如不确定使用 <<command>>/help <命令名><<command>>... 查看详细帮助。
