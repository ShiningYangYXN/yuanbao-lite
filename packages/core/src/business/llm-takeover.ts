...
const DEFAULT_SYSTEM_PROMPT = `你是元宝Lite智能助手，一个友好、专业的AI聊天机器人，在腾讯元宝平台生活，这个平台的用户有时也把“群聊”称作“派”。
...
注意，生成回复时，请不要添加消息标头，输出回复本体即可，系统会帮你添加。
...
示例：
  ...
  用户说"帮我发个表情" → <<sticker>>关键词<<sticker>> 直接发送贴纸（先搜索匹配，再发送到当前会话）
  用户说"发个狗头" → <<sticker>>狗头<<sticker>>
  用户问"刚才谁说了什么" → <<command>>/search history 关键词<<command>>... 搜索，再总结结果

## 贴纸发送

你可以在回复中嵌入贴纸标签来直接发送贴纸到当前会话（私聊或群聊）：

  <<sticker>>关键词<<sticker>>

系统会按关键词搜索贴纸（支持名称和描述模糊匹配），找到后发送到当前会话。
如果没找到匹配的贴纸，会反馈错误。

例如：
  <<sticker>>狗头<<sticker>>      — 发送"狗头"贴纸
  <<sticker>>666<<sticker>>        — 发送"666"贴纸
...
### 危险模式
某些命令权限较高，通常仅限私聊使用。在群聊中使用这些命令需要：
- /unsafe on [分钟] — 开启危险模式（默认5分钟），允许所有高权限命令在群聊使用
...
- 命令名可加/也可不加，支持别名（如 v = version = /version）
- 不可授权命令: unsafe, trust, block, config, init, daemon, shell
...
- 使用<<command>>/command args<<command>>格式执行命令时，两侧的<<command>>和命令斜杠缺一不可。如需查看命令结果后进行下一步操作，必须在命令后面添加...，比如：<<command>>/help<<command>>...
...
以下是你可用的全部命令。命令名必须加/前缀。标记 [仅私聊] 的命令在群聊中需 unsafe 模式或授权。系统使用回复触发者ID对命令进行鉴权。
...
### 聊天与贴纸
/chat <目标> <消息> [仅私聊] — 发送消息（自动识别群聊/私聊：9位纯数字=群，其他=私聊）
...
/search <groups|members|history> <关键词> — 搜索群组/成员/消息历史

### 历史
/history recent [数量] — 当前会话最近消息
/history search <关键词> — 搜索历史
/search history [--all] <关键词> — 搜索历史（合并自 /hsearch，--all 显示全部及完整文本）
...
export class LlmTakeoverEngine {
    ...
    if (bot) {
      ...
      if (cmdSys) {
        ...
        if (cmdLines.length > 0) {
          systemPrompt += `\n\n=== 可用命令 ===\n${cmdLines.join("\n")}\n=== 命令结束 ===`;
          if (unsafe) {
            systemPrompt += `\n\n你可以通过在回复中嵌入 <<command>>/命令名 参数<<command>> 来执行命令。⚠️ 危险模式已开启，所有命令（包括elevated命令）均可在群聊中使用。`;
          } else {
            systemPrompt += `\n\n你可以通过在回复中嵌入 <<command>>/命令名 参数<<command>> 来执行命令。例如: <<command>>/ping<<command>> 或 <<sticker>>狗头<<sticker>>。在群聊中，elevated命令不可用。`;
          }
        }
      }
    }
  ...
  // ─── Sticker Trigger Mechanism ───

  /**
   * Handle <<sticker>>keyword<<sticker>> tags in LLM output.
   *
   * Searches the sticker registry by keyword (fuzzy match on name + description),
   * sends the best match to the current conversation (group or DM that the
   * message originated from), and strips the tag from the reply text.
   *
   * Returns the text with all <<sticker>> tags removed, and a list of
   * human-readable status lines (one per tag) to append to the reply.
   */
  private async handleStickerTriggers(
    bot: YuanbaoBot,
    msg: ChatMessage,
    text: string,
  ): Promise<{ cleanedText: string; results: string[] }> {
    const stickerPattern = /<<sticker>>\s*(.+?)\s*<<sticker>>/g;
    const matches = [...text.matchAll(stickerPattern)];
    if (matches.length === 0) {
      return { cleanedText: text, results: [] };
    }
    ...
    for (const match of matches) {
      ...
      cleanedText = cleanedText.replace(match[0], "");
      this.log.info(`LLM sticker trigger: "${keyword}"`);

      try {
        const hits = searchStickers(keyword, 1);
        if (hits.length === 0) {
          results.push(`🚫 贴纸未找到: "${keyword}"`);
          continue;
        }
        const sticker = hits[0];
        const to =
          msg.chatType === "group" && msg.groupCode
            ? msg.groupCode
            : msg.fromUserId;
        ...
        results.push(`🎭 已发送贴纸: ${sticker.name} (${sticker.id})`);
      } catch (err) {
        results.push(
          `🚫 贴纸发送失败 ("${keyword}"): ${(err as Error).message}`,
        );
      }
    }
  ...
  private async executeIterativeInvoke(
    ...
    while (true) {
      ...
      try {
        ...
        // Handle <<sticker>>keyword<<sticker>> triggers (sends stickers,
        // strips tags, appends status lines)
        const stickerResult = await this.handleStickerTriggers(
          bot,
          msg,
          processedText,
        );
        if (stickerResult.results.length > 0) {
          processedText = stickerResult.cleanedText
            ? `${stickerResult.cleanedText}\n\n${stickerResult.results.join("\n")}`
            : stickerResult.results.join("\n");
        }
