  ...
  cmdSys.register({
    ...
    handler: async (ctx) => {
      ...
      if (exactMatch) {
        ...
        const candidates = recentMsgs.filter(
          (m) => m.id && String(m.id).endsWith(msgId!),
        );
      ...
      try {
        ...
        await ctx.reply(
          `✅ 已引用回复消息 #${msgId!.length > 8 ? msgId!.slice(-8) : msgId}`,
        );
      } catch (err) {
