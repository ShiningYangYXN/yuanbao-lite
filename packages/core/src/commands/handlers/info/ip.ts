...
export function register(cmdSys: CommandSystem): void {
  cmdSys.register({
    ...
    handler: async (ctx) => {
      ...
      if (!isIPv4 && !isIPv6) {
        await ctx.reply(
          "❌ 无效 IP 地址 (支持 IPv4 如 8.8.8.8 或 IPv6 如 2001:4860:4860::8888)",
        );
        return;
      }
      ...
      const providers: Array<() => Promise<IpResult>> = [
        ...
        async () => {
          ...
          try {
            const resp = await fetch(
              `http://ip-api.com/json/${encodeURIComponent(ip)}?lang=zh-CN&fields=status,message,country,regionName,city,isp,as,timezone,lat,lon`,
            ...
            if (d.status === "fail")
              throw new Error(String(d.message ?? "fail"));
            return {
              provider: "ip-api.com",
        ...
        async () => {
          ...
          try {
            ...
            const [lat, lon] =
              typeof d.loc === "string" ? d.loc.split(",") : ["", ""];
            return {
              provider: "ipinfo.io",
              country: String(d.country ?? ""),
              region: String(d.region ?? ""),
