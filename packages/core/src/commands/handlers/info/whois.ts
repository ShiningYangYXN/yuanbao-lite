    ...
    handler: async (ctx) => {
      ...
      try {
        ...
        const lines = [
          ...
          ...(registration
            ? [
                `  注册时间: ${new Date(registration.eventDate).toLocaleString("zh-CN")}`,
              ]
            : []),
          ...(expiration
            ? [
                `  到期时间: ${new Date(expiration.eventDate).toLocaleString("zh-CN")}`,
              ]
            : []),
          ...(lastChanged
            ? [
                `  最后更新: ${new Date(lastChanged.eventDate).toLocaleString("zh-CN")}`,
              ]
            : []),
          ...(status.length > 0
            ? [
                `  状态: ${status.slice(0, 3).join(", ")}${status.length > 3 ? ` (+${status.length - 3})` : ""}`,
              ]
            : []),
        ];
