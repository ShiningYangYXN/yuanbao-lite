    ...
    handler: async (ctx) => {
      ...
      try {
        ...
        if (
          ...
          const suffix =
            !ctx.showAll && members.member_list.length > 50
          ...
          if (ctx.useTable) {
            const rows = memberList.map((m) => {
              ...
              const isOwner =
                groupOwnerId && String(m.user_id) === String(groupOwnerId);
              const labels = [isSelf ? "本体" : "", isOwner ? "群主" : ""]
                .filter((l) => l)
                .join(" ");
              return [m.nick_name || m.user_id, m.user_id, typeLabel, labels];
            });
            await ctx.reply(
              `👥 群成员 (${members.member_list.length}人):\n${await ctx.formatTable(["昵称", "用户ID", "类型", "标记"], rows)}${suffix}`,
            );
          } else {
            const lines = memberList.map((m) => {
              ...
              const labels = [selfLabel, ownerLabel, typeLabel]
                .filter((l) => l)
                .join(" ");
