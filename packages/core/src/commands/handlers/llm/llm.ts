...
export function register(cmdSys: CommandSystem): void {
  cmdSys.register({
    ...
    usage:
      "/llm <on|off|status|billing|chat|prompt|model|temp|history|clear|provider|config|customprovider|raw|im|group|merge|cooldown|iterate> [参数]",
      ...
      if (!subCmd) {
        ...
        if (engine) {
          ...
          const kv: [string, string][] = [
            ...
            [
              "最大迭代轮数",
              config.maxIterate === 0 ? "无限" : String(config.maxIterate),
            ],
      ...
      switch (subCmd) {
        ...
        case "账单": {
          ...
          if (recent.length > 0) {
            ...
            for (const r of recent) {
              ...
              lines.push(
                `  ${time} — ${r.provider}/${r.model}: ${r.totalTokens} tokens (${r.promptTokens}+${r.completionTokens})`,
              );
            }
          }
          await ctx.reply(lines.join("\n"));
        ...
        case "自定义供应商": {
          ...
          if (action === "remove") {
            ...
            if (!customProviders[subArgs[1]]) {
              await ctx.reply(`未找到: ${subArgs[1]}`);
              return;
            }
          ...
          if (action === "removekey") {
            if (subArgs.length < 3) {
              await ctx.reply(
                "用法: /llm customprovider removekey <名称> <索引>",
              );
              return;
            }
            const name = subArgs[1];
            ...
            if (isNaN(idx) || idx < 0 || idx >= pool.length) {
              await ctx.reply(`无效索引 (0-${pool.length - 1})`);
              return;
            }
            pool.splice(idx, 1);
            customProviders[name].apiKeys = pool;
        ...
        case "配置": {
          ...
          const llmWizardSessions = (
            cmdSys as unknown as { _llmWizardSessions?: Map<string, unknown> }
          )._llmWizardSessions;
          ...
          setTimeout(
            () => {
              const session = llmWizardSessions.get(sessionKey) as
                | { startedAt: number }
                | undefined;
        ...
        case "合并": {
          ...
          if (subArgs.length === 0) {
            await ctx.reply(
              `当前合并窗口: ${cfg.mergeWindowMs}ms (0=不等待，立即响应)`,
            );
          } else {
        ...
        case "迭代": {
          ...
          if (subArgs.length === 0) {
            await ctx.reply(
              `当前最大迭代轮数: ${cfg.maxIterate === 0 ? "无限" : cfg.maxIterate} (0=无限)`,
            );
          } else {
          ...
          if (ctx.source === "cli") {
            ...
            const confirmations = (
              cmdSys as unknown as {
                _llmResetConfirmations?: Map<
                  string,
                  { count: number; firstAt: number }
                >;
              }
            )._llmResetConfirmations;
            if (!confirmations) {
            ...
            if (entry.count < REQUIRED) {
              await ctx.reply(
                `⚠️ 确认清空LLM配置 (${entry.count}/${REQUIRED})\n还需 ${REQUIRED - entry.count} 次确认`,
              );
              return;
            }
            // Confirmed — execute
  ...
  (
    cmdSys as unknown as {
      _llmResetConfirmations: Map<string, { count: number; firstAt: number }>;
    }
  )._llmResetConfirmations = new Map();
  ...
  (
    cmdSys as unknown as { _llmWizardSessions: Map<string, unknown> }
  )._llmWizardSessions = llmWizardSessions;
  ...
  (
    ...
    if (session.step === "apiFormat") {
      ...
      API_FORMATS.forEach((f, i) => {
        ...
        if (f.value === "openai-chat-completions") {
          formatMap["openai"] = f.value;
          formatMap["1"] = f.value;
        }
        if (f.value === "anthropic-messages") {
          formatMap["anthropic"] = f.value;
          formatMap["claude"] = f.value;
          formatMap["2"] = f.value;
        }
        if (f.value === "google-gemini-rest") {
          formatMap["gemini"] = f.value;
          formatMap["google"] = f.value;
          formatMap["3"] = f.value;
        }
        if (f.value === "aws-bedrock-converse") {
          formatMap["bedrock"] = f.value;
          formatMap["aws"] = f.value;
          formatMap["4"] = f.value;
        }
        if (f.value === "azure-openai") {
          formatMap["azure"] = f.value;
          formatMap["5"] = f.value;
        }
      });
    ...
    if (session.step === "apiKey") {
      ...
      const config = engine.getConfig() as {
