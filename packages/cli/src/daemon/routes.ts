...
import type { YuanbaoBot } from "@yuanbao-lite/core";
import type { ChatMessage } from "@yuanbao-lite/core/types";
import type { CliProfile } from "@yuanbao-lite/core/shared/config";
import { getVersion } from "@yuanbao-lite/core/version";
import { resolve } from "node:path";
import { existsSync } from "node:fs";
import { getGlobalContactStore } from "@yuanbao-lite/core/business/contacts";
...
>>>>>>> b1b50d2 (refactor: v12.10.0 — merge sticker search into /search, unify search logic, fix wizard sessionKey, promote generic helpers)
  ...
  return {
    ...
    body: {
      ok: true,
      active: hasInit || hasLlm || hasTerm,
      wizard: hasInit ? "init" : hasLlm ? "llm" : hasTerm ? "term" : null,
    },
  };
}
