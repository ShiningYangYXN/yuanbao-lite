...
export async function runInitWizard(): Promise<void> {
  ...
  p.log.message(
    chalk.dim("首次使用需要配置认证信息 (appKey + appSecret 或 token)"),
  );
  ...
  const appSecret = await p.text({
    ...
    placeholder: "",
    validate: (val) =>
      !val || !val.trim() ? "App Secret 不能为空" : undefined,
  });
  ...
  p.log.success(
    chalk.green(`✓ 配置已保存到: ${store.getConfigDir()}/config.json`),
  );
  ...
  if (!process.env.YB_DAEMON_CHILD) {
    printError(
      "提示: 如需让新配置生效，请重启 daemon (yb-cli daemon stop && yb-cli daemon start)",
    );
  }
}
